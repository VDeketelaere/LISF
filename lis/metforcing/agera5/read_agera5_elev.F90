...
!
! !INTERFACE:
subroutine read_agera5_elev(n,findex)
! !USES:
...
! !DESCRIPTION:
!
!  Opens and reads AgERA5 model elevation to the LIS grid.
...
!EOP
  ...
  if ( LIS_rc%met_ecor(findex) == "lapse-rate" .or. &
     ...
     write(LIS_logunit,*) '[INFO] Reading the AgERA5 elevation map '
     
     call LIS_read_param(n,"ELEVATION",go)
